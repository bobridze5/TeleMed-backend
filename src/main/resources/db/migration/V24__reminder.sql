CREATE TABLE reminder (
    ...
    reminder_recurrence_days VARCHAR(120),
    reminder_enabled BOOLEAN NOT NULL DEFAULT TRUE,
    reminder_last_fired_at TIMESTAMPTZ,
    reminder_created_at TIMESTAMPTZ,
    reminder_updated_at TIMESTAMPTZ,
    CONSTRAINT fk_reminder_patient FOREIGN KEY (patient_id) REFERENCES patients (patient_id) ON DELETE CASCADE,
    CONSTRAINT ck_reminder_kind CHECK (
        reminder_kind IN ('ONE_SHOT', 'RECURRING')
    ),
    CONSTRAINT ck_reminder_kind_fields CHECK (
        (
        ...
        OR (
            ...
            AND reminder_recurrence_time IS NOT NULL
            AND reminder_recurrence_days IS NOT NULL
        )
    )
);
...
CREATE INDEX idx_reminder_kind_enabled ON reminder (
    reminder_kind,
    reminder_enabled
);