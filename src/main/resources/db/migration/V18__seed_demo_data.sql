...
VALUES ('Москва', (SELECT region_id FROM regions WHERE region_name = 'Москва и Московская область'))
ON CONFLICT (city_name, region_id) DO NOTHING;

-- ──────────────────────────────────────────────
-- Медицинская организация
-- ──────────────────────────────────────────────
INSERT INTO medical_organizations (organization_name, organization_address, organization_email, organization_phone, organization_city)
VALUES (
    'Городская клиника №1',
    'г. Москва, ул. Академика Сахарова, д. 12',
    'clinic1@telemed.ru',
    '+7 (495) 123-45-67',
    (SELECT city_id FROM cities WHERE city_name = 'Москва')
);
...
WITH u AS (
    INSERT INTO users (user_first_name, user_last_name, user_middle_name,
                       user_email, user_password_hash, user_status)
    VALUES ('Главный', 'Администратор', NULL,
            'admin@telemed.ru',
...
INSERT INTO patients (patient_id, patient_diabetes_type,
                      patient_target_low, patient_target_high,
                      patient_insulin_dependency,
                      patient_blood_type, patient_height_cm)
...
WITH u AS (
    INSERT INTO users (user_first_name, user_last_name, user_middle_name,
                       user_date_of_birth, user_gender,
                       user_email, user_password_hash, user_status)
    VALUES ('Анна', 'Смирнова', 'Владимировна',
            '1992-07-22', 'F',
...
WITH u AS (
    INSERT INTO users (user_first_name, user_last_name, user_middle_name,
                       user_date_of_birth, user_gender,
                       user_email, user_password_hash, user_status)
    VALUES ('Михаил', 'Волков', 'Андреевич',
            '1978-11-05', 'M',
...
WITH u AS (
    INSERT INTO users (user_first_name, user_last_name, user_middle_name,
                       user_date_of_birth, user_gender,
                       user_email, user_password_hash, user_status)
    VALUES ('Алексей', 'Кузнецов', 'Иванович',
            '1975-06-10', 'M',
...
INSERT INTO doctors (doctor_id, doctor_specialization_id,
                     doctor_experience, doctor_qualification,
                     doctor_organization_id, doctor_about)
SELECT u.user_id,
       (SELECT specialization_id FROM specializations WHERE specialization_name = 'Эндокринолог'),
       15,
       'Высшая категория',
       (SELECT organization_id FROM medical_organizations WHERE organization_email = 'clinic1@telemed.ru'),
...
WITH u AS (
    INSERT INTO users (user_first_name, user_last_name, user_middle_name,
                       user_date_of_birth, user_gender,
                       user_email, user_password_hash, user_status)
    VALUES ('Елена', 'Морозова', 'Сергеевна',
            '1980-02-14', 'F',
...
INSERT INTO doctors (doctor_id, doctor_specialization_id,
                     ...
                     doctor_organization_id, doctor_about)
SELECT u.user_id,
       (SELECT specialization_id FROM specializations WHERE specialization_name = 'Диабетолог'),
       12,
       'Первая категория',
       (SELECT organization_id FROM medical_organizations WHERE organization_email = 'clinic1@telemed.ru'),
...
WITH u AS (
    INSERT INTO users (user_first_name, user_last_name, user_middle_name,
                       user_date_of_birth, user_gender,
                       user_email, user_password_hash, user_status)
    VALUES ('Дмитрий', 'Соколов', 'Александрович',
            '1988-09-30', 'M',
...
INSERT INTO doctors (doctor_id, doctor_specialization_id,
                     ...
                     doctor_about)
SELECT u.user_id,
       (SELECT specialization_id FROM specializations WHERE specialization_name = 'Диетолог'),
       8,
       'Вторая категория',
       'Составляет индивидуальные планы питания для пациентов с сахарным диабетом. Опыт 8 лет.'
FROM u;
