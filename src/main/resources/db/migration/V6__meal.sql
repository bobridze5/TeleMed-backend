...
CREATE TABLE nutrition_day (
    ...
    CONSTRAINT fk_nutrition_day_patient
        FOREIGN KEY (patient_id)
        REFERENCES patients(patient_id)
        ON DELETE CASCADE,
...
CREATE TABLE meal (
    ...
    CONSTRAINT fk_meal_patient
        ...
        REFERENCES patients(patient_id)
        ON DELETE CASCADE,
    CONSTRAINT fk_meal_nutrition_day
        FOREIGN KEY (nutrition_day_id)
        REFERENCES nutrition_day(nutrition_day_id)
        ON DELETE CASCADE
);
...
CREATE TABLE meal_item (
    ...
    CONSTRAINT ck_meal_item_nutrition_nonneg CHECK (
        meal_item_portion_grams >= 0 AND
        meal_item_calories_per_100 >= 0 AND
        meal_item_carbs_per_100 >= 0 AND
        meal_item_protein_per_100 >= 0 AND
        meal_item_fats_per_100 >= 0 AND
        meal_item_carbs_per_100_bread_unit >= 0 AND
        meal_item_quantity > 0
    )
);
