CREATE TABLE patient_doctor_assignments (
    ...
    -- Уникальность пары пациент-врач
    CONSTRAINT uq_patient_doctor UNIQUE (patient_id, doctor_id),

    -- Внешние ключи (убедитесь, что названия таблиц patients и doctors верны)
...
-- Индексы для оптимизации поиска (особенно если часто проверяются активные назначения)
