...
-- 3. Индексы для ускорения поиска (полезно для расписания)
CREATE INDEX idx_appointment_doctor_date ON appointments(doctor_id, appointment_datetime);
CREATE INDEX idx_appointment_patient_date ON appointments(patient_id, appointment_datetime);
